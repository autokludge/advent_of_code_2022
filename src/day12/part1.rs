    ...
    for (x_shift, y_shift) in address_shift {
        ...
        if let Some(n_val) = grid.get(n_y, n_x) {
            ...
            if *n_val as i32 - *current_val as i32 <= 1 {
                ...
                // heuristics
                //let elevation = 27 - *n_val as u64;
                //let manhattan_distance = goal.x.abs_diff(n_x as i32) + goal.y.abs_diff(n_y as i32);

                let h = 0; //elevation + manhattan_distance as u64;
                neighbours.push((p, h));
            }
        }
    }
...
fn a_star(start: &Point2d, goal: &Point2d, input: HeightMap) -> Option<Vec<Point2d>> {
    ...
    open_set.push(PriorityPoint {
        point: start.clone(),
        f_score: *h_values.get(start.x as usize, start.y as usize).unwrap() as u64, /* hardcoded 27, but need a function */
    });
...
pub fn solve(input: &Input) -> Output {
    ...
    let goal = &input.end();
    let path = a_star(start, goal, input.clone());

    let steps = path.unwrap().iter().count() - 1;

    Output::U64(steps as u64)
}
